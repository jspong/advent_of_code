    ...
    for n in range(6):
        ...
        for i in range(*x):
            for j in range(*y):
                for k in range(*z):
                    for m in range(*w):
                        new_positions[(i,j,k,m)] = step(positions, i, j, k, m)
...
def step(positions, x, y, z, w):
    if positions[(x,y,z, w)]:
        return int(neighbors(positions, x, y, z, w) in (2, 3))
    else:
        return int(neighbors(positions, x, y, z, w) == 3)

def neighbors(positions, x, y, z, w):
    count = 0
    for i in (-1, 0, 1):
        for j in (-1, 0, 1):
            for k in (-1, 0, 1):
                for m in (-1, 0, 1):
                    if i == 0 and j == 0 and k == 0 and m == 0:
                        continue
                    count += positions[(x+i, y+j, z+k, w+m)]
    return count
